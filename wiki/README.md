# Wiki

Quick reference pages for day-to-day operations.

## Navigation

### Daily Operations
- [Daily Checklist](./pages/daily-checklist.md) — Morning, mid-day, evening tasks
- [Pricing Calculator](./pages/pricing-calculator.md) — Formulas and quick tables
- [Seasonal Calendar](./pages/seasonal-calendar.md) — When demand peaks
- [Competitor Tracker](./pages/competitor-tracker.md) — Top sellers, pricing gaps

### Products & Sourcing
- [Vehicle Brands Reference](./pages/vehicle-brands.md) — Hino, Isuzu, Mitsubishi specs
- [Isuzu Catalog System](./pages/isuzu-catalog-system.md) — Official dealer parts catalog access
- [Product Catalog Index](./pages/product-catalog-index.md) — Top SKUs with prices
- [Sourcing Guide](./pages/sourcing-guide.md) — Where to buy, MOQ, quality tiers

### Customers & Sales
- [B2B Customer Personas](./pages/b2b-customer-personas.md) — Fleets, workshops

### Platform & Seller Academy
- [Tokopedia & TikTok Shop Seller Academy](./pages/tokopedia-tiktok-seller-academy.md) — Complete platform rules, policies, fees

### Research Repository
- See [../research/](../research/) for full pricing, demand, and strategy files.

### Concurrent Research
- [Concurrent Research Findings](./pages/research-concurrent-findings.md) — Chinese trucks, Euro 4, B50 mandate, TikTok scan
- [Pricing Intel Template](./pages/pricing-intel-template.md) — Ready for product list integration

---

## Quick Stats

| Category | Count |
|----------|-------|
| Research files | 21 |
| Wiki pages | 12 |
| Vehicle brands | 3 (Hino, Isuzu, Mitsubishi) |
| Product categories | 6 |
| Catalog systems | 1 (Isuzu i-Parts Center) |
| Concurrent research | 5 tracks active |

---

## Workflow

### When Pricing Changes
1. Update `research/18-wholesale-margin-analysis.md`
2. Update `wiki/pages/pricing-calculator.md`
3. Update `wiki/pages/competitor-tracker.md`
4. Commit and push

### When Adding New Products
1. Add to `research/14-engine-parts-catalog.md` (full data)
2. Add to `wiki/pages/product-catalog-index.md` (quick ref)
3. Update `wiki/pages/seasonal-calendar.md` (demand timing)
4. Commit and push

---

## Last Updated

- Wiki structure: April 9, 2026
- Exchange rate: Rp 16.000/USD
- Platform fees: Current as of Q1 2026
